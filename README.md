...
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg)](https://github.com/kuyun/kuyun/pulls)
[![GitHub stars](https://img.shields.io/github/stars/kuyun/kuyun.svg?style=social&label=Stars)](https://github.com/kuyun/kuyun)
[![GitHub forks](https://img.shields.io/github/forks/kuyun/kuyun.svg?style=social&label=Fork)](https://github.com/kuyun/kuyun)

交流QQ群：133107819🈵、284280411🈵、305155242♨️、528049386🈵(群内含各种工具和文档下载)

## 前言

　　`kuyun`项目创建于2016年10月4日，正在慢慢成长中，目的不仅仅是一个开发架构，而是努力打造一套从 **前端模板** - **基础框架** - **分布式架构** - **开源项目** - **持续集成** - **自动化部署** - **系统监测** - **无缝升级** 的全方位J2EE企业级开发解决方案。
...
![组织结构](project-bootstrap/kuyun_project.png)
...
kuyunAdmin | 后台管理系统模板  | [https://github.com/kuyun/kuyunAdmin](https://github.com/kuyun/kuyunAdmin)
autoMail | 邮箱地址自动补全插件  | [https://github.com/kuyun/autoMail](https://github.com/kuyun/autoMail)
kuyun.jprogress.js | 加载进度条插件  | [https://github.com/kuyun/kuyun.jprogress.js](https://github.com/kuyun/kuyun.jprogress.js)
kuyun.jtotop.js | 返回顶部插件  | [https://github.com/kuyun/kuyun.jtotop.js](https://github.com/kuyun/kuyun.jtotop.js)
...
基于bootstrap实现的响应式Material Design风格的通用后台管理系统，`kuyun`项目所有后台系统都是使用该模块界面作为前端展示。
...
![API网关](project-bootstrap/kuyun-api.png)
...
[环境搭建和系统部署文档(作者：小兵)](http://git.oschina.net/kuyun/kuyun/attach_files "环境搭建和系统部署文档(作者：小兵)")
...
- dubbo-admin-2.5.3 [http://download.csdn.net/detail/kuyun5201314/9733652](http://download.csdn.net/detail/kuyun5201314/9733652 "dubbo-admin-2.5.3")
- dubbo-admin-2.5.4-SNAPSHOT-jdk8 [http://download.csdn.net/detail/kuyun5201314/9733657](http://download.csdn.net/detail/kuyun5201314/9733657 "dubbo-admin-2.5.4-SNAPSHOT-jdk8")
- 更多资源请加QQ群
...
- 127.0.0.1	ui.kuyun.cn
- 127.0.0.1	upms.kuyun.cn
- 127.0.0.1	cms.kuyun.cn
- 127.0.0.1	pay.kuyun.cn
- 127.0.0.1	ucenter.kuyun.cn
- 127.0.0.1	wechat.kuyun.cn
- 127.0.0.1	api.kuyun.cn
- 127.0.0.1	oss.kuyun.cn

### 编译流程

kuyun-admin、kuyun-common => kuyun-upms => 其他

### 启动顺序（后台）
```
- 新建kuyun数据库，导入project-datamodel文件夹下的kuyun.sql

- 修改各dao模块和rpc-service模块的redis.properties、jdbc.properties、generator.properties数据库连接等配置信息，其中master.redis.password、master.jdbc.password、slave.jdbc.password、generator.jdbc.password密码值使用了AES加密，请使用com.kuyun.common.util.AESUtil工具类修改这些值

- 首先启动 kuyun-upms-rpc-service(直接运行src目录下的kuyunUpmsRpcServiceApplication#main方法启动) => kuyun-upms-server(jetty)，然后按需启动对应子系统xxx的kuyun-xxx-rpc-service(main方法) => kuyun-xxx-webapp(jetty)
```
![启动演示](project-bootstrap/start.png)
```
- 访问 [统一后台地址 http://upms.kuyun.cn:1111/]，子系统菜单已经配置到kuyun-upms权限中，不用直接访问子系统，默认帐号密码：admin/123456
...
```
- 启动nginx代理kuyun-ui静态资源，配置文件可参考 [nginx.conf](http://git.oschina.net/kuyun/kuyun/attach_files)
...
- rpc-service服务提供者jar包：将打包后的kuyun-xxx-rpc-service-assembly.tar.gz文件解压，使用bin目录的管理脚本运行即可，支持优雅停机。
```
...
- 模块命名为`项目`-`子项目`-`业务`，如`kuyun-cms-admin`

- 数据表命名为：`子系统`_`表`，如`cms_article`

- 更多规范，参考[[阿里巴巴Java开发手册] http://git.oschina.net/kuyun/kuyun/attach_files

```

## 演示地址

演示地址： [http://www.kuyun.cn/kuyunAdmin](http://www.kuyun.cn/kuyunAdmin "演示地址")

### 预览图
![idea](project-bootstrap/idea.png)
![login](project-bootstrap/kuyun-login.png)
![upms](project-bootstrap/kuyun-upms.png)
![cms](project-bootstrap/kuyun-cms.png)
![swagger](project-bootstrap/api.png)

### 数据模型
![数据库模型](project-datamodel/kuyun.png)
...
首先谢谢大家支持，如果你希望参与开发，欢迎通过[Github](https://github.com/kuyun/kuyun "Github")上fork本项目，并Pull Request您的commit。
...
- 报kuyun-xxx.jar包找不到,请按照文档编译顺序，将源代码编译并安装到本地maven仓库

- kuyun-cms-admin启动卡住：因为没有启动activemq

- kuyun-upms-server访问报session不存在：因为没有启动redis服务

- 界面没有样式：因为kuyun-admin没有编译安装到本地仓库

## 附件

### 优秀文章和博客

- [创业互联网公司如何搭建自己的技术框架](http://kuyun5201314.iteye.com/blog/2330151 "创业互联网公司如何搭建自己的技术框架")

- [微服务实战](https://segmentfault.com/a/1190000004634172 "微服务实战")

- [单点登录原理与简单实现](http://kuyun5201314.iteye.com/blog/2343910 "单点登录原理与简单实现")
...
- [支付系统架构](http://kuyun5201314.iteye.com/blog/2355431 "支付系统架构")
...
- [做个男人，做个成熟的男人，做个有城府的男人](http://kuyun5201314.iteye.com/blog/1387820 "做个男人，做个成熟的男人，做个有城府的男人")
