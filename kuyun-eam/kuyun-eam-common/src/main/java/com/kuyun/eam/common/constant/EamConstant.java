package com.kuyun.eam.common.constant;

import com.kuyun.common.base.BaseConstants;

/**
 * cms系统常量类
 * Created by kuyun on 2017/2/19.
 */
public class EamConstant extends BaseConstants {

    public static String MAINTAIN_DEPARTMENT="维修部";

    public static String TICKET_CREATE = "ticketCreate";
    public static String TICKET_REPAIR = "ticketRepair";
    public static String TICKET_APPOINT = "ticketAppoint";

    public static String TICKET_APPOINT_ACCEPT = "接单";
    public static String TICKET_APPOINT_REJECT = "拒绝";

    public static String MAINTAIN_PLAN_LABEL = "维护计划";
}
