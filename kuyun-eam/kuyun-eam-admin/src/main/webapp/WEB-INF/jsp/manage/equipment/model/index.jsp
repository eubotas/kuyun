...
<div id="main">
	<div id="toolbar">
		<shiro:hasPermission name="eam:equipmentModel:create"><a class="waves-effect waves-button" href="javascript:;" onclick="createAction()"><i class="zmdi zmdi-plus"></i> 新增设备模型</a></shiro:hasPermission>
...
$(function() {
	...
	$table.bootstrapTable({
		url: '${basePath}/manage/equipment/model/list',
		height: getHeight(),
		striped: true,
		search: true,
		showRefresh: true,
...
function createAction() {
	createDialog = $.dialog({
		...
		title: '新增设备模型',
		columnClass: 'xlarge',
		content: 'url:${basePath}/manage/equipment/model/create',
...
function updateAction() {
	...
	if (rows.length != 1) {
		...
		updateDialog = $.dialog({
			...
			title: '编辑设备模型',
			columnClass: 'xlarge',
			content: 'url:${basePath}/manage/equipment/model/update/' + rows[0].equipmentModelId,
			onContentReady: function () {
				initMaterialInput();
				$('select').select2();
			}
...
function viewModelPropertyAction() {
    var rows = $table.bootstrapTable('getSelections');
    if (rows.length != 1) {
        $.confirm({
            title: false,
			...
			buttons: {
				confirm: {
					...
					action: function () {
						var ids = new Array();
						for (var i in rows) {
							ids.push(rows[i].equipmentModelId);
						}
						$.ajax({
							type: 'get',
							url: '${basePath}/manage/equipment/model/delete/' + ids.join("-"),
