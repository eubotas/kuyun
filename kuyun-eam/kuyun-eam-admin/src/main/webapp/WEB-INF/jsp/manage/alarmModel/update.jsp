		...
		<div class="row">
			<div class="col-sm-12">
				<div class="form-group">
					<div class="fg-line">
						...
						<input id="name" type="text" class="form-control" name="name" maxlength="20" value="${alarmModel.name}">
					</div>
				</div>
			</div>
			<div class="col-sm-12">
					...
					<div class="fg-line">
						<select id="eamDataElementId" name="eamDataElementId" style="width: 100%">
							<c:forEach var="eamDataElement" items="${eamDataElements}">
								<option value="${eamDataElement.id}" ${alarmModel != null && alarmModel.eamDataElementId == eamDataElement.id ? 'selected' : ''}>${eamDataElement.lableName}</option>
							</c:forEach>
						</select>
					</div>
				</div>
					...
					<div class="fg-line">
						<select id="alarmType" name="alarmType" style="width: 100%">
							<c:forEach var="alarmType" items="${alarmTypes}">
								<option value="${alarmType.code}" ${alarmModel != null && alarmModel.alarmType == alarmType.code ? 'selected' : ''}>${alarmType.name}</option>
							</c:forEach>
						</select>
					</div>
				</div>
			...
			<div id="upperBoundDiv">
				<div class="col-sm-4">
					<div class="form-group">
						<div class="fg-line">
							<input id="upperBound" type="text" class="form-control" name="upperBound" maxlength="20" placeholder="X" <c:if test="${alarmModel != null}"> value="${alarmModel.upperBound}"</c:if>>
						</div>
					</div>
				</div>
			</div>
			...
			<div id="lowerBoundDiv" style="display:none">
				<div class="col-sm-4">
					<div class="form-group">
						<div class="fg-line">
							<input id="lowerBound" type="text" class="form-control" name="lowerBound" maxlength="20" placeholder="Y" <c:if test="${alarmModel != null}"> value="${alarmModel.lowerBound}"</c:if>>
						</div>
					</div>
				</div>
			</div>
			...
			<div id="durationDiv" style="display:none">
				<div class="col-sm-4">
					<div class="form-group">
						<div class="fg-line">
							<input id="duration" type="text" class="form-control" name="duration" maxlength="20" placeholder="M" <c:if test="${alarmModel != null}"> value="${alarmModel.duration}"</c:if>>
						</div>
					</div>
				</div>
			</div>
...
function updateSubmit() {
    $.ajax({
        type: 'post',
        url: '${basePath}/manage/alarmModel/update/${alarmModel.alarmModelId}',
