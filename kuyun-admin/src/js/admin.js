...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('kuyun-upms-systemid', systemid);
		$.cookie('kuyun-upms-systemname', systemname);
		$.cookie('kuyun-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('kuyun-upms-systemid') || 1;
	var systemname = $.cookie('kuyun-upms-systemname') || 'kuyun-upms-server';
	var systemtitle = $.cookie('kuyun-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
