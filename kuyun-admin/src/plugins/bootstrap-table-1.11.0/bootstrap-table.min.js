    ...
    $.each(this.options.columns, function (i, columns) {
        ...
        $.each(columns, function (j, column) {
            ...
            html.push('<th' + sprintf(' title="%s"', column.titleTooltip),
                ...
                sprintf(' data-sortName="%s"', column.sortName), //one
                "tabindex='0'",
                '>');
...
$.fn.bootstrapTable.Constructor.prototype.getCaret = function () {
    ...
    $.each(this.$header.find('th'), function (i, th) {
        ...
        var result = $(th).data('sortName') == undefined ? $(th).data('field') === that.options.sortName : $(th).data('sortName') === that.options.sortName;
        $(th).find('.sortable').removeClass('desc asc').addClass(result ? that.options.sortOrder : 'both'); //two
    });
};
...
$.fn.bootstrapTable.Constructor.prototype.onSort = function (event) {
    ...
    if ($this.data('sortName') == undefined){ //three
        this.options.sortName = $this.data('field');
    } else {
        this.options.sortName = $this.data('sortName');
    }
    this.options.sortOrder = $this.data('order') === 'asc' ? 'asc' : 'desc';
    this.trigger('sort', this.options.sortName, this.options.sortOrder);

    $this.add($this_).data('order', this.options.sortOrder === 'asc' ? 'desc' : 'asc');
